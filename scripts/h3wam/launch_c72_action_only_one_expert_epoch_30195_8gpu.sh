...
project="${C72_SOURCE_SNAPSHOT:?C72 long training requires a complete read-only source snapshot}"
freeze_sha="${C72_SOURCE_FREEZE_SHA256:?Set reviewed SOURCE_FREEZE.json SHA256}"
release_file="${C72_RELEASE_FILE:?C72 requires a hash-bound long-run release JSON}"
canary_gate="${C72_CANARY_GO_LONG:?C72 requires the passed canary GO_LONG.json}"
python_bin="${PYTHON_BIN:-${workspace}/runtime/h3-int8-native/bin/python}"
output_root="${OUTPUT_ROOT:-${workspace}/outputs/c72-action-only-one-expert-epoch-v1/online-long30195-v1}"
...
finalizer="${project}/scripts/h3wam/finalize_c72_action_only_one_expert_epoch.py"
launcher="${project}/scripts/h3wam/launch_c72_action_only_one_expert_epoch_30195_8gpu.sh"
...
for path in "${python_bin}" "${project}/SOURCE_FREEZE.json" "${release_file}" \
  ...
  [[ -e "${path}" ]] || { echo "missing C72 long input: ${path}" >&2; exit 2; }
done
...
if (
    canary.get("status") != "GO_C72_LONG"
    or canary.get("permission") != "MECHANICAL_PERMISSION_ONLY"
    or not all(canary.get("gate", {}).values())
):
    raise SystemExit("C72 canary did not authorize long training")
if (
    ...
    raise SystemExit("C72 fixed C58b parent gate failed")
keys = (
...
fixed = {
    "format": "h3wam-c72-action-only-one-expert-epoch-release-v1",
    "status": "GO_C72_ACTION_ONLY_30195",
    "permission": "MANUAL_GPU_RELEASE",
    "optimizer_steps": 30195,
    "scheduler_horizon": 30195,
...
if mismatches or sha(freeze_path) != freeze_sha:
    raise SystemExit("C72 long release mismatch: " + ",".join(mismatches))
PY

[[ ! -e "${output_root}" ]] || { echo "refusing existing C72 long output" >&2; exit 2; }
free_bytes="$(df -PB1 "${workspace}" | awk 'NR==2 {print $4}')"
[[ "${free_bytes}" =~ ^[0-9]+$ && "${free_bytes}" -ge 483183820800 ]] || {
  echo "C72 requires at least 450 GiB free; found ${free_bytes:-unknown}" >&2; exit 2;
}
...
export TMPDIR="${workspace}/tmp/c72-action-only-long"
mkdir -p "${TMPDIR}"
cd "${project}"
...
common=(
  ...
  --base-lr 2e-5 --action-lr 2e-4 --warmup-steps 500 --scheduler-horizon 30195
...
for milestone in "${milestones[@]}"; do
  ...
  checkpoint="${output_root}/checkpoints/c72_action_only_s${milestone}.pt"
