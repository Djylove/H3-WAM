...
project="${C69_SOURCE_SNAPSHOT:?C69 canary requires a complete read-only source snapshot}"
...
output_root="${OUTPUT_ROOT:-${workspace}/outputs/c69-matched-action-only-v1/canary10-v1}"
checkpoint="${output_root}/c69_action_only_s10.pt"
...
for path in "${python_bin}" "${project}/SOURCE_FREEZE.json" \
  ...
  "${project}/experiments/dossiers/h3_c69_matched_action_only_v1.json" \
  "${c58_parent}" "${c58_ready}"; do
  [[ -e "${path}" ]] || { echo "missing C69 canary input: ${path}" >&2; exit 2; }
done
[[ "$(sha256sum "${project}/SOURCE_FREEZE.json" | awk '{print $1}')" == "${source_freeze_sha}" ]] || {
  echo "C69 source freeze SHA mismatch" >&2; exit 2;
}
...
[[ ! -e "${output_root}" ]] || { echo "refusing existing C69 canary output" >&2; exit 2; }
mkdir -p "${output_root}"
...
export TMPDIR="${workspace}/tmp/c69-action-only-canary"
mkdir -p "${TMPDIR}"
cd "${project}"
...
checkpoint = root / "c69_action_only_s10.pt"
...
marker = {
    "format": "h3wam-c69-matched-action-only-canary-v1",
    "status": "GO_C69_LONG" if all(gate.values()) else "NO_GO_C69_LONG",
    ...
    "claim_boundary": "Optimizer/DDP/freeze/restore proof only; no action or LIBERO effect claim.",
}
(root / "GO_LONG.json").write_text(json.dumps(marker, indent=2) + "\n")
print(json.dumps(marker, sort_keys=True))
if marker["status"] != "GO_C69_LONG":
    raise SystemExit(64)
PY
