...
output_root="${C24_OUTPUT_ROOT:-${workspace}/eval/c24-first-action-horizon-sweep-v1}"
checkpoint="${workspace}/outputs/dense-carrier-d0-h32-s20000-v1/checkpoints/d0_h32_s14000.pt"
shard="${C24_SHARD:?set C24_SHARD to 0, 1, 2, or 3}"
horizon="${C24_HORIZON:?set C24_HORIZON to 16 or 32}"
[[ "${shard}" =~ ^[0-3]$ ]]
[[ "${horizon}" == 16 || "${horizon}" == 32 ]]
candidate_root="${output_root}/h${horizon}"
test -f "${output_root}/selection.jsonl"
test ! -e "${candidate_root}/shard${shard}.COMPLETED"
...
for gpu in {0..7}; do
  IFS=$'\t' read -r ordinal suite task trial index distance first_seed offset continuation_seed trajectory \
    <<< "${selection[${gpu}]}"
  slug="${suite#libero_}"
  run_root="${candidate_root}/runs/${ordinal}_${slug}_task${task}_trial${trial}_d${distance}_offset${offset}"
  env H3_WORKSPACE="${workspace}" PROJECT_ROOT="${project}" SUITE="${suite}" \
    ...
    CONTINUATION_POLICY_NOISE_SEED_BASE="${continuation_seed}" FIRST_REPLAN_STEPS="${horizon}" \
    OUTPUT_ROOT="${run_root}" \
    "${project}/scripts/h3wam/run_dense_d0_milestone_rollout.sh" \
    H32 "${checkpoint}" "${gpu}" "${task}" "${trial}" \
    >"${candidate_root}/logs/${ordinal}_${slug}_task${task}_trial${trial}_d${distance}_offset${offset}.log" 2>&1 &
  pids+=("$!")
done
started="$(date +%s)"
failed=0
...
(( failed == 0 ))
printf '{"horizon":%s,"shard":%s,"branches":8,"duration_seconds":%s}\n' \
  "${horizon}" "${shard}" "$(( $(date +%s) - started ))" > "${candidate_root}/shard${shard}.COMPLETED"
