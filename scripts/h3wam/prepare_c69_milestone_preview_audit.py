#!/usr/bin/env python3
"""Audit one completed C69 milestone for preview-only balanced80 evaluation."""
...
FINAL = load_sibling("_c69_preview_finalizer", "finalize_c69_matched_action_only_20k.py")
FORMAT = "h3wam-c69-milestone-preview-audit-v1"
...
def prepare(root: Path, milestone: int) -> dict[str, Any]:
    ...
    if milestone not in FINAL.MILESTONES:
        raise ValueError("C69 milestone must be s1000..s20000")
    ...
    if audit.get("checkpoint_sha256") != sha256_file(checkpoint):
        raise ValueError("C69 milestone checkpoint SHA drift")
    return {
        "format": FORMAT,
        ...
        "claim_boundary": (
            "Preview-only evaluation of one completed action-only milestone. It "
            "cannot alter C69, select a checkpoint, authorize rollout, or establish "
            "the C67-vs-C69 mechanism effect."
        ),
    }
