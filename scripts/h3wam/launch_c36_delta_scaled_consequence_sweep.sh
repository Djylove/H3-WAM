...
: "${C36_RUN_INDEX:?set C36_RUN_INDEX to 0..3}"
[[ "$C36_RUN_INDEX" =~ ^[0-3]$ ]] || { echo "C36_RUN_INDEX must be 0..3" >&2; exit 2; }
...
output_root="/mnt/h3-wam/outputs/c36-delta-scaled-consequence-v1"
python_bin="/mnt/h3-wam/runtime/conda-py311/bin/python"
dossier="$project/experiments/dossiers/h3_c36_delta_scaled_consequence_v1.json"

cd "$project"
for artifact in "$c34_root/dataset.pt" "$c34_root/h3_features.pt" "$dossier"; do
  [[ -s "$artifact" ]] || { echo "required C36 artifact absent: $artifact" >&2; exit 3; }
done
...
variant="${variants[$C36_RUN_INDEX]}"
seed="${seeds[$C36_RUN_INDEX]}"
run_name="${variant}_seed${seed}"
mkdir -p "$output_root/$run_name/checkpoints"

export LD_LIBRARY_PATH=/usr/local/nvidia/lib:/usr/local/nvidia/lib64
export CUDA_VISIBLE_DEVICES="${C36_GPU_INDEX:-0}"
...
exec "$python_bin" scripts/h3wam/train_c31_action_conditioned_consequence.py \
  ...
  --experiment-id h3_c36_delta_scaled_consequence_v1
