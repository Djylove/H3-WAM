...
project="${C73_PREVIEW_SOURCE_SNAPSHOT:?C73 preview requires a reviewed immutable snapshot}"
freeze_sha="${C73_PREVIEW_SOURCE_FREEZE_SHA256:?Set reviewed C73 preview SOURCE_FREEZE SHA256}"
train_root="${C73_TRAIN_ROOT:-${workspace}/outputs/c73-action-only-three-expert-epoch-v1/online-long130585-v1}"
root="${C73_PREVIEW_ROOT:?C73_PREVIEW_ROOT is required}"
...
milestones=(30195 34000)

[[ ! -e "${root}" ]] || { echo "refusing existing C73 preview root: ${root}" >&2; exit 2; }
for path in "${python_bin}" "${project}/SOURCE_FREEZE.json" "${verifier}" \
  ...
  [[ -e "${path}" ]] || { echo "missing C73 preview input: ${path}" >&2; exit 2; }
done
...
for milestone in "${milestones[@]}"; do
  for path in \
    "${train_root}/checkpoints/c73_action_only_s${milestone}.pt" \
    "${train_root}/reports/train_s${milestone}.json" \
    "${train_root}/restore/restore_s${milestone}.json"; do
    [[ -s "${path}" ]] || { echo "missing completed C73 s${milestone} input: ${path}" >&2; exit 2; }
  done
done
...
export TMPDIR="${workspace}/tmp/c73-balanced80-deadline-preview"
mkdir -p "${TMPDIR}"
...
wait "${p0}" || status=1
wait "${p1}" || status=1
(( status == 0 )) || { echo "one or more C73 preview workers failed" >&2; exit 1; }
...
for step in (30195, 34000):
    ...
    if (
        ...
        raise SystemExit(f"invalid C73 preview report: s{step}")
    reports[str(step)] = hashlib.sha256(path.read_bytes()).hexdigest()
marker = {
    "format": "h3wam-c73-action-only-deadline-preview-complete-v1",
    "status": "PASS_C73_S30195_S34000_PREVIEWS_COMPLETE",
    "permission": "DIAGNOSTIC_ONLY_CONTINUE_FIXED_TRAJECTORY",
    "effect_status": "NOT_EVIDENCE_READY",
    "reports_sha256": reports,
    "claim_boundary": "No checkpoint selection, rollout, early stop, or budget-effect claim.",
}
