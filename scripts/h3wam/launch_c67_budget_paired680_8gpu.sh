...
project="${PROJECT_ROOT:?PROJECT_ROOT must be the complete read-only C67 source snapshot}"
root="${C67_ROLLOUT_ROOT:?C67_ROLLOUT_ROOT must contain AUTHORIZATION.json and jobs.jsonl}"
...
for path in "${project}/SOURCE_FREEZE.json" "${authorization}" "${manifest}" \
  ...
  [[ -e "${path}" ]] || { echo "missing C67 rollout source/input: ${path}" >&2; exit 2; }
done
[[ ! -e "${root}/COMPLETED.json" ]] || { echo "C67 rollout already complete" >&2; exit 2; }
[[ ! -e "${root}/INVALID.json" ]] || { echo "C67 rollout root is invalid" >&2; exit 2; }

freeze_sha="$(${sim_python} - "${authorization}" <<'PY'
import json, sys
value=json.load(open(sys.argv[1]))
print(value.get("source_freeze",{}).get("sha256", ""))
PY
)"
...
"${sim_python}" - "${authorization}" "${manifest}" "${project}" \
  "${source_manifest}" "${train_manifest}" "${cache_root}/stats.pt" \
  "${c48_dataset}" "${c48_observations}" "${c59_root}/COMPLETED.json" \
  "${c59_root}/sample_labels.jsonl" "${h3_checkpoint}" <<'PY'
...
data_paths = list(map(Path, sys.argv[4:11]))
h3 = Path(sys.argv[11])
...
if auth.get("format")!="h3wam-c67-budget-rollout-authorization-v1" or \
 auth.get("status")!="AUTHORIZED_C67_S10_S20_PAIRED_680" or \
 auth.get("permission")!="GO_C67_1360_FRESH_PROCESSES_NO_INTERMEDIATE_STOP" or \
 auth.get("release_signed") is not False or auth.get("historical_c60_data_sha256")!=expected_data:
    raise SystemExit("C67 authorization contract mismatch")
if [sha(path.resolve()) for path in data_paths] != [expected_data[key] for key in ordered]:
    raise SystemExit("C67 historical seven-data SHA fail-close")
if sha(h3.resolve())!="e889202c41dafb67b10d67b97f0d8541508036a6090af23425a5c2615d03c47a":
    raise SystemExit("C67 H3 checkpoint SHA mismatch")
if sha(manifest_path.resolve())!=auth.get("manifest_sha256") or len(jobs)!=1360:
    raise SystemExit("C67 job manifest mismatch")
expected={(a,s,t,r) for r in range(33,50) for s in
 ("libero_spatial","libero_object","libero_goal","libero_10") for t in range(10)
 for a in ("matched_control","treatment")}
actual={(j.get("arm"),j.get("suite"),j.get("tasks",[None])[0],j.get("trials",[None])[0])
 for j in jobs if j.get("episodes")==1}
if actual!=expected or len(actual)!=len(jobs): raise SystemExit("C67 1360-grid mismatch")
for endpoint in auth["endpoints"].values():
    ...
    if not path.is_file() or sha(path)!=endpoint["checkpoint_sha256"]:
        raise SystemExit("C67 endpoint bytes mismatch")
if stat.S_IMODE(project.stat().st_mode)&0o222:
    raise SystemExit("C67 source snapshot root is writable")
PY

lock="${root}/.launcher.lock"
mkdir "${lock}" 2>/dev/null || { echo "another C67 launcher owns ${lock}" >&2; exit 75; }
...
run_worker() {
  ...
  while IFS=$'\t' read -r job_id pair_id arm suite task trial assigned_gpu checkpoint output; do
    ...
    [[ ! -e "${output}" ]] || {
      echo "refusing partial/reused C67 output: ${output}" >&2; return 1;
    }
    mkdir -p "${output}"
    log="${root}/logs/job$(printf '%04d' "${job_id}")_pair$(printf '%03d' "${pair_id}")_${arm}.log"
    ...
    bash "${project}/scripts/h3wam/run_cloud_libero.sh" \
      ...
      --c67-budget-rollout-authorization "${authorization}" \
...
for line in open(sys.argv[1]):
 ...
 print("\t".join(map(str,(row["job_id"],row["pair_id"],row["arm"],row["suite"],
  row["tasks"][0],row["trials"][0],row["gpu"],row["checkpoint"],row["output"]))))
PY
)
}
...
(( status == 0 )) || { echo "one or more C67 paired workers failed" >&2; exit 1; }

"${sim_python}" - "${authorization}" "${manifest}" "${root}/COMPLETED.json" "${authorization_sha}" <<'PY'
import hashlib,json,os,sys
from pathlib import Path
auth=json.load(open(sys.argv[1])); rows=[json.loads(x) for x in open(sys.argv[2]) if x.strip()]
auth_sha=sys.argv[4]
if len(rows)!=1360: raise ValueError("C67 isolated job count mismatch")
seen=set()
for row in rows:
 ...
 if key in seen or len(found)!=1 or data.get("task_ids")!=row["tasks"] or data.get("trial_indices")!=row["trials"]:
  raise ValueError(f"invalid/duplicate isolated C67 job: {path}")
 if data.get("c67_budget_rollout_authorization_sha256")!=auth_sha:
  raise ValueError(f"C67 result authorization mismatch: {path}")
 seen.add(key)
if len(seen)!=1360: raise ValueError("C67 completed identity count mismatch")
out=Path(sys.argv[3]); tmp=out.with_name(f".{out.name}.{os.getpid()}.partial")
tmp.write_text(json.dumps({
 "format":"h3wam-c67-budget-paired680-isolated-complete-v1","status":"COMPLETE",
 "jobs":1360,"pairs":680,"episodes_per_arm":680,
 "authorization_sha256":auth_sha,"manifest_sha256":auth["manifest_sha256"],
},indent=2)+"\n")
os.replace(tmp,out)
PY
