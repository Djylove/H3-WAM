...
project="${C72_SOURCE_SNAPSHOT:?C72 canary requires a complete read-only source snapshot}"
...
output_root="${OUTPUT_ROOT:-${workspace}/outputs/c72-action-only-one-expert-epoch-v1/canary10-v1}"
checkpoint="${output_root}/c72_action_only_s10.pt"
...
for path in "${python_bin}" "${project}/SOURCE_FREEZE.json" "${verifier}" "${trainer}" \
  "${project}/experiments/dossiers/h3_c72_action_only_one_expert_epoch_v1.json" \
  "${c58_parent}" "${c58_ready}"; do
  [[ -e "${path}" ]] || { echo "missing C72 canary input: ${path}" >&2; exit 2; }
done
[[ "$(sha256sum "${project}/SOURCE_FREEZE.json" | awk '{print $1}')" == "${freeze_sha}" ]] || {
  echo "C72 source freeze SHA mismatch" >&2; exit 2;
}
...
[[ ! -e "${output_root}" ]] || { echo "refusing existing C72 canary output" >&2; exit 2; }
mkdir -p "${output_root}"
...
export TMPDIR="${workspace}/tmp/c72-action-only-canary"
mkdir -p "${TMPDIR}"
cd "${project}"
...
gate = {
    ...
    "one_expert_epoch_schedule": contract.get("scheduler_horizon") == 30195,
...
checkpoint = root / "c72_action_only_s10.pt"
...
marker = {
    "format": "h3wam-c72-action-only-one-expert-epoch-canary-v1",
    "status": "GO_C72_LONG" if all(gate.values()) else "NO_GO_C72_LONG",
