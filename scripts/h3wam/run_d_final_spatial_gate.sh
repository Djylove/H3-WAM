...
log_dir="${workspace}/eval/d-final-spatial-gate-v1/${checkpoint_name}"
mkdir -p "${log_dir}"
test -f "${checkpoint}"
...
for trial in 0 1; do
  for task in {0..9}; do
    ...
    if [[ -f "${output_root}/results.json" ]]; then
      ...
      [[ ! -e "${output_root}" ]] || { echo "refusing partial output ${output_root}" >&2; exit 1; }
    ...
    gpu=$((gpu + 1))
    if (( gpu == 8 )); then wait_wave; gpu=0; fi
  done
done
if (( ${#wave_pids[@]} )); then wait_wave; fi
...
for trial in (0, 1):
    for task in range(10):
        root = workspace / "outputs/eval-d-spatial" / f"{name}_spatial_task{task}_trial{trial}_replan8"
...
report = {
    "format": "h3-d-final-spatial-gate-v1", "checkpoint": name,
    "tasks": list(range(10)), "trials": [0, 1], "replan_steps": 8,
    "successes": sum(row["success"] for row in rows), "episodes": len(rows),
    "fixed_parent": "7/20", "promotion": "candidate must exceed 7/20",
    "records": rows,
}
output = workspace / "eval/d-final-spatial-gate-v1" / f"{name}_summary.json"
