...
output_root="${C27_OUTPUT_ROOT:-${workspace}/eval/c27-expanded-causal-dataset-v1}"
checkpoint="${workspace}/outputs/dense-carrier-d0-h32-s20000-v1/checkpoints/d0_h32_s14000.pt"
shard="${C27_SHARD:?set C27_SHARD to 0, 1, 2, or 3}"
[[ "${shard}" =~ ^[0-3]$ ]]
...
printf '{"shard":%s,"branches":%s,"duration_seconds":%s}\n' \
  "${shard}" "${#selection[@]}" "$(( $(date +%s) - started ))" > "${output_root}/shard${shard}.COMPLETED"
