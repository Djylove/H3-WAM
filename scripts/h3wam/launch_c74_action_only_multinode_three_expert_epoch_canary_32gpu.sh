...
project="${C74_SOURCE_SNAPSHOT:?C74 canary requires a complete read-only source snapshot}"
...
output_root="${OUTPUT_ROOT:-${workspace}/outputs/c74-action-only-multinode-three-expert-epoch-v1/canary10-v1}"
checkpoint="${output_root}/c74_action_only_s10.pt"
...
master_port="${C74_MASTER_PORT:-29574}"
[[ "${nnodes}" == 4 && "${node_rank}" =~ ^[0-3]$ ]] || {
  echo "C74 canary requires four nodes with node rank 0..3" >&2; exit 2;
}

for path in "${python_bin}" "${project}/SOURCE_FREEZE.json" "${verifier}" "${trainer}" \
  "${project}/experiments/dossiers/h3_c74_action_only_multinode_three_expert_epoch_v1.json" \
  "${c58_parent}" "${c58_ready}"; do
  [[ -e "${path}" ]] || { echo "missing C74 canary input: ${path}" >&2; exit 2; }
done
[[ "$(sha256sum "${project}/SOURCE_FREEZE.json" | awk '{print $1}')" == "${freeze_sha}" ]] || {
  echo "C74 source freeze SHA mismatch" >&2; exit 2;
}
...
if [[ "${node_rank}" == 0 ]]; then
  [[ ! -e "${output_root}" ]] || { echo "refusing existing C74 canary output" >&2; exit 2; }
  mkdir -p "${output_root}"
...
PY
  ...
  for _ in $(seq 1 120); do [[ -f "${prepared}" ]] && break; sleep 1; done
  [[ -f "${prepared}" ]] || { echo "timed out waiting for C74 canary preparation" >&2; exit 2; }
fi
...
export TMPDIR="${workspace}/tmp/c74-action-only-canary-node${node_rank}"
mkdir -p "${TMPDIR}"
cd "${project}"
...
restore_port=$((master_port + 1))
"${python_bin}" -m torch.distributed.run --nnodes "${nnodes}" --nproc-per-node 8 \
  --node-rank "${node_rank}" --master-addr "${master_addr}" --master-port "${restore_port}" \
...
if [[ "${node_rank}" != 0 ]]; then
  for _ in $(seq 1 300); do [[ -f "${output_root}/GO_LONG.json" ]] && exit 0; sleep 1; done
  echo "timed out waiting for C74 canary result" >&2
  exit 2
fi
...
gate = {
    ...
    "three_expert_epoch_schedule": contract.get("scheduler_horizon") == 32647,
    ...
    ] * 4 and contract.get("rank_schedule") == {
        "name": "c67_4_2_1_1", "group_size": 8, "group_repetitions": 4,
    },
    "sample_equivalent_batch_scaling": contract.get("base_lr") == 8e-5
    and contract.get("action_lr") == 8e-4 and contract.get("warmup_steps") == 125,
...
checkpoint = root / "c74_action_only_s10.pt"
...
marker = {
    "format": "h3wam-c74-action-only-multinode-three-expert-epoch-canary-v1",
    "status": "GO_C74_LONG" if all(gate.values()) else "NO_GO_C74_LONG",
