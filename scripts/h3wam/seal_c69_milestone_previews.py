#!/usr/bin/env python3
"""Rebind C69 preview metrics to its final 20k evidence without inference."""
...
PREVIEW_AUDIT_FORMAT = "h3wam-c69-milestone-preview-audit-v1"
REPORT_FORMAT = "h3wam-c69-action-only-milestone-balanced80-v1"
COMPLETE_FORMAT = "h3wam-c69-matched-action-only-training-complete-v1"
PREVIEWS_COMPLETE_FORMAT = "h3wam-c69-action-only-preview-complete-v1"
...
def _training_complete(path: Path) -> tuple[dict[str, Any], dict[int, dict[str, Any]]]:
    ...
    if (
        ...
        or complete.get("status") != "PASS_C69_MATCHED_ACTION_ONLY_TRAINING_COMPLETE"
...
def seal(
    ...
    if (
        ...
        or preview_marker.get("status") != "PASS_C69_ALL_20_PREVIEWS_COMPLETE"
        or preview_marker.get("permission") != "WAIT_FOR_FIXED_C67_VS_C69_AGGREGATION"
        ...
        raise ValueError("C69 preview-complete marker failed before sealing")
    ...
    for milestone in MILESTONES:
        ...
        checkpoint = (train_root / f"checkpoints/c69_action_only_s{milestone}.pt").resolve()
        checkpoint_sha = sha256_file(checkpoint)
        if (
            embedded.get("checkpoint_sha256") != checkpoint_sha
            or Path(embedded.get("checkpoint", "")).resolve() != checkpoint
            or preview_audit.get("format") != PREVIEW_AUDIT_FORMAT
            or preview_audit.get("status") != "PASS_C69_MILESTONE_PREVIEW_AUDIT"
            ...
            raise ValueError(f"C69 preview/final audit mismatch: s{milestone}")
        arm = report.get("arm", {})
        gates = report.get("conditioning_gates", {})
        if (
            report.get("format") != REPORT_FORMAT
            or report.get("variant") != "c69"
            ...
            raise ValueError(f"invalid C69 preview report binding: s{milestone}")
    ...
    manifest = {
        ...
        "claim_boundary": (
            ...
            "is evaluated. Only the fixed C67-s20000 versus C69-s20000 cross-arm "
