#!/usr/bin/env python3
"""Fail-closed audit of the C72 fresh 30,195-step action-only trajectory."""
...
READY_FORMAT = "h3wam-c72-action-only-one-expert-epoch-complete-v1"
MILESTONES = tuple(range(1_000, 30_001, 1_000)) + (30_195,)
...
def require_contract(contract: dict[str, Any]) -> None:
    ...
    if mismatches:
        raise ValueError(f"C72 fixed contract mismatch: {mismatches}")
    if contract.get("initialization") != {
        "initialization_contract": "strict_online_c58b_parent_v1",
        "c58_completed_steps": 10_000,
    }:
        raise ValueError("C72 fixed C58b initialization mismatch")
    ...
    if (
        ...
        raise ValueError("C72 auxiliary freeze contract mismatch")
    for name in (
        ...
        if not isinstance(value, str) or len(value) != 64:
            raise ValueError(f"C72 missing data identity: {name}")


def expected_lr_factor(step: int) -> float:
    if step < 500:
        return float(step + 1) / 500
    progress = min(1.0, (step - 500) / 29_695)
...
def validate_milestone(
    ...
    checkpoint = root / f"checkpoints/c72_action_only_s{milestone}.pt"
    ...
    if expected_contract is not None and contract != expected_contract:
        raise ValueError(f"C72 contract drift at s{milestone}")
    previous_step = previous_milestone(milestone)
    previous = (
        None if previous_step == 0
        else root / f"checkpoints/c72_action_only_s{previous_step}.pt"
    )
    ...
    if failed:
        raise ValueError(f"C72 s{milestone} gate failed: {','.join(failed)}")
    ...
    if failed:
        raise ValueError(f"C72 s{milestone} checkpoint gate failed: {','.join(failed)}")
    return {
        "milestone": milestone,
        "status": "PASS_C72_MILESTONE_STRICT_RESTORE",
...
def main() -> None:
    ...
    report = {
        "format": READY_FORMAT,
        "status": "PASS_C72_ACTION_ONLY_ONE_EXPERT_EPOCH_COMPLETE",
        "permission": "READY_FOR_PREREGISTERED_OFFLINE_ONLY",
        "effect_status": "NOT_EVIDENCE_READY",
        "completed_steps": 30_195,
        "training_samples": 241_560,
        "expert_training_samples": 120_780,
        "incremental_expert_effective_epochs": 120_780 / 200_779,
        "cumulative_expert_effective_epochs_with_c58_parent": (
            80_000 + 120_780
        ) / 200_779,
        "primary_internal_control": audits[MILESTONES.index(20_000)]["checkpoint"],
        ...
        "claim_boundary": (
            ...
            "C72-s20000 versus C72-s30195 offline and C69 champion paired LIBERO remain unproven."
        ),
    }
