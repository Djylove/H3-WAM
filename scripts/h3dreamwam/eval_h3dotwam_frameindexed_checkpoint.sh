...
ACTION_STAGE="${1:?usage: eval_h3dotwam_frameindexed_checkpoint.sh ACTION_STAGE LABEL}"
LABEL="${2:?usage: eval_h3dotwam_frameindexed_checkpoint.sh ACTION_STAGE LABEL}"
...
TRAIN_MANIFEST="${H3_WORKSPACE}/data/v8_multisuite_frameindexed_candidate/manifest_train_uniform.jsonl"
# Keep the episode-disjoint v7 validation set as a fixed external measurement.
VAL_MANIFEST="${H3_WORKSPACE}/data/v7_multisuite_dense_candidate/manifest_val_stratified40.jsonl"
OUTPUT_ROOT="${H3_WORKSPACE}/outputs/eval-frameindexed-dot/m11/${LABEL}"
TMP_ROOT="${H3_WORKSPACE}/tmp/frameindexed-eval-${LABEL}"
...
"${PYTHON_BIN}" -m torch.distributed.run --standalone --nproc_per_node=8 \
  ...
  --load-stage "${ACTION_STAGE}" --eval-only --steps 5 --sample-steps 10 \
  --action-horizon 32 --require-text-only-context --log-every 1 \
...
bash "${PROJECT_ROOT}/scripts/h3wam/run_cloud_libero.sh" \
  ...
  --max-steps 400 --wait-steps 30 --replan-steps 10 --action-horizon 32 \
  --sample-steps 10 --output-dir "${OUTPUT_ROOT}/libero_goal_task3" \
...
rollout = json.loads((root / "libero_goal_task3/results.json").read_text())
record = {"action_stage": str(stage.resolve()), "val_action_loss": val["mean_action_loss"],
          "successes": rollout["successes"], "episodes": rollout["episodes"],
