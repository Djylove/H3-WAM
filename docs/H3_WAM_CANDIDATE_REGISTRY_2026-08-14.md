...
| C31 | consequence/model | action-conditioned future-H3 adapter | C30 passed causal consequences | FACT clean-action K/V + MiniWorld 4-action/latent alignment | flattened与8-token temporal同数据同预算；correct、同状态错配、zero-action三臂 | 代码与9项测试通过；等待C30 data gate与610个live INT8 H3特征 | HOLD_DATA_GATE；禁止提前训练/排名 |
